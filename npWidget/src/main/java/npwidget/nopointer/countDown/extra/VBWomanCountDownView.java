package npwidget.nopointer.countDown.extra;
...
/**
 * 倒计时 VBand 生理周期所用的view
 */
public class VBWomanCountDownView extends BaseView {
    ...
    public VBWomanCountDownView(Context context) {
        super(context);
        init(context);
    }

    public VBWomanCountDownView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public VBWomanCountDownView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context) {
        ...
        dialLength = SizeUtils.dp2px(context, 4);
        dialWidth = SizeUtils.dp2px(context, 3);
    ...
    private void drawInSide() {
        ...
        if (outSideStyle == OutSideStyle.DIAL) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                canvas.drawLine(viewRectF.left + outSideMargin + dialLength*5, viewRectF.centerY(), viewRectF.left + outSideMargin + dialLength+dialLength*6, viewRectF.centerY(), paint);
                canvas.restore();
            }
        } else {
            //圆环样式
...
//        canvas.drawCircle(viewRectF.centerX(), viewRectF.centerY(), 20, paint);
        ...
        canvas.drawCircle(progressBarCenterX, progressBarCenterY, progressBarRadius-circleWidth/3, paint);
//        paint.setStyle(Paint.Style.STROKE);
...
//        }
     ...
     * @param second
     */
    public void startCountDown(int second, float progress) {
        if (endAnimator != null) {
            endAnimator.removeAllUpdateListeners();
            endAnimator.cancel();
        }
        ...
        startAnimator = ObjectAnimator.ofFloat(0, progress);
        startAnimator.setDuration(second * 1000);
        startAnimator.setInterpolator(new DecelerateInterpolator());
        startAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
